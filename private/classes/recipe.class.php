  ...
  public function __construct($args = [])
    {
        ...
        $this->recipe_total_servings = $args['recipe_total_servings'] ?? 1;
        $this->recipe_post_date = $args['recipe_post_date'] ?? date('Y-m-d H:i:s');
    ...
    protected function validate() {
        $this->errors = [];
            ...
            if (is_blank($this->recipe_name)) {
                $this->errors['recipe_name'][] = "Recipe name cannot be blank.";
            } elseif (!has_length($this->recipe_name, ['min' => 2, 'max' => 40])) {
                $this->errors['recipe_name'][] = "Recipe name must be between 2 and 40 characters.";
            } elseif (!preg_match("/^[A-Za-z0-9\-'\s]+$/", $this->recipe_name)) {
                $this->errors['recipe_name'][] = "Recipe name can only contain letters, numbers, hyphens, apostrophes, and spaces.";
            }
            ...
            if (is_blank($this->recipe_total_servings)) {
                $this->errors['recipe_total_servings'][] = "Total servings cannot be blank.";
            } elseif (!is_numeric($this->recipe_total_servings) || $this->recipe_total_servings <= 0) {
                $this->errors['recipe_total_servings'][] = "Total servings must be a positive number.";
            } elseif ($this->recipe_total_servings > 99) {
                $this->errors['recipe_total_servings'][] = "Total servings cannot exceed 99.";
            }

            if (empty($this->recipe_difficulty)) {
                $this->errors['recipe_difficulty'][] = "Recipe difficulty cannot be blank.";
            } elseif (!in_array($this->recipe_difficulty, ['beginner', 'intermediate', 'advanced'])) {
                $this->errors['recipe_difficulty'][] = "Invalid recipe difficulty level.";
            }
